...
via [brew](https://github.com/homebrew-ffmpeg/homebrew-ffmpeg/):
...
It also tries to fix timestamps which are very short, and can be extended (due
to there not being a subtitle in the frame before). Using median also means the
outputs will be idempotent.
