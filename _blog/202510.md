...
Constrained generation is something that has interested me recently. Mostly as an extension of structured generation.
For example in the newest `gpt-5` models you can now have Regex as a constrained output. Now
[outlines](https://github.com/dottxt-ai/outlines) is not particularly new, though what is interesting to me is the
design of their APIs.
...
Putting this together, we can attempt to get the LLM to play chess (albeit not very well). It does make me wonder how we
may use LLMs to play games in a precise manner, e.g. if moves are injected in context to 'promote' them or dissuade an
LLM from playing bad moves.
